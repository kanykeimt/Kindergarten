...
@section('content')
    <div class="col-12">
        <button type="button" class="btn btn-primary" id="addUserBtnId" onclick="showForm()">@lang('lang.add_child')</button>
        <div class="d-none" id="addUserId" class="bg-light rounded h-100 p-4">
            <h6 class="mb-4">@lang('lang.create_child_form')</h6>
            <form action="{{route('employee.children.create')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
        ...
        <div class="bg-light rounded h-100 p-4">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody id="childTable">
                    @php $child_index = 1; @endphp
                    @foreach ($children as $child)
                        <tr class=>
                            <td class="">{{$child_index}}</td>
                            <td class="">{{$child->name}}</td>
                            <td class="">{{$child->surname}}</td>
                            <td class="">{{$child->parent->name}} {{$child->parent->surname}}</td>
                            <td>
                                <div style="float: left;
                                display: block;
                                width: 33%;" class="text-center">
                                    <a href="{{route('employee.children.show', $child)}}"><i class="fa fa-info me-2"></i></a>
                                </div>
                                <div style="float: left;
                                display: block;
                                width: 33%;" class="text-center">
                                    <a href="{{route('employee.children.edit', $child)}}" class="text-success"><i class="fas fa-pen me-2"></i></a>
                                </div>
                                <div style="float: left;
                                display: block;
                                width: 33%;" class="text-center">
                                    <form action="{{route('employee.children.delete', $child->id)}}" method="POST">
                                        @method('DELETE')
                                        @csrf
